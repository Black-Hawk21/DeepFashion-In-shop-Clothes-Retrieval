...
def load_blip2():
    ...
    Used for two things in the online pipeline:
      1. Caption generation on the query crop (fused into the query embedding).
      2. ITM re-ranking: score each candidate caption against the query image.
    """
...
def encode_query_image(image, clip_model, preprocess):
    ...
    return F.normalize(emb.float(), dim=-1)  # returns tensor for optional fusion


@torch.no_grad()
def generate_caption(image, blip2_processor, blip2_model):
    """Generate a BLIP-2 caption for the query crop (same prompt used during offline indexing)."""
    prompt = "A photo of a clothing item:"
    inputs = blip2_processor(
        images=image, text=prompt, return_tensors="pt"
    ).to(blip2_model.device, torch.float16)
    output_ids = blip2_model.generate(**inputs, max_new_tokens=50)
    caption = blip2_processor.decode(output_ids[0], skip_special_tokens=True)
...
def encode_caption(caption, clip_model):
    """Encode the generated caption with CLIP text encoder."""
    import clip as clip_lib
    tokens = clip_lib.tokenize([caption], truncate=True).to(DEVICE)
    emb = clip_model.encode_text(tokens)
    return F.normalize(emb.float(), dim=-1)


def fuse_embeddings(img_emb_tensor, txt_emb_tensor, alpha):
    """Fuse image and text embeddings: v = alpha*img + (1-alpha)*txt, L2-normalised."""
    fused = alpha * img_emb_tensor + (1.0 - alpha) * txt_emb_tensor
...
st.markdown(
    ...
    **Pipeline (per project spec):**
    ...
    2. **BLIP-2 caption** — generate a caption for the query crop
    3. **CLIP (fused)** — encode crop + caption, fuse with alpha weighting
...
with st.sidebar:
    ...
    fusion_alpha = st.slider(
        "Fusion alpha (α·image + (1-α)·caption)",
        min_value=0.0, max_value=1.0, value=0.5, step=0.05,
        help="α=1.0 → image only. α=0.0 → caption only. Match the alpha used when building your index."
    )
    ...
    st.markdown(f"**Fusion α:** `{fusion_alpha}`")
    ...
    st.markdown("4. BLIP-2 generates caption for crop")
    st.markdown(f"5. CLIP encodes crop + caption, fused at α={fusion_alpha}")
    st.markdown("6. HNSW retrieves candidate pool")
    st.markdown("7. BLIP-2 ITM re-ranks by image↔caption score")
    st.markdown("---")
...
if detections:
    # --- Build individual clothing crops ---
    crop_cards = [
        {"crop": query_image.crop(d["bbox"]), "bbox": d["bbox"], "conf": d["conf"], "label": d["label"]}
        for d in detections
    ]

    # --- Build full-body crop: union bounding box of all detections ---
    ...
    if crop_cards:
        st.markdown("**Individual clothing crops:**")
        ...
        for i, card in enumerate(crop_cards):
            with grid_cols[i % n_cols]:
                ...
                st.image(
                    card["crop"],
                    caption=f"Crop {i + 1} · {card['label']} (conf={card['conf']:.2f}, {w}×{h})",
                    use_container_width=True,
                )

    # --- Display full-body crop ---
    st.markdown("**Full-body crop** (union of all clothing regions):")
    st.image(full_body_crop, width=250,
             caption=f"Full-body crop · {fb_w}×{fb_h}")

    # --- Radio: individual crops + full-body option ---
    # Options: 0..N-1 = individual crops, N = full-body
    FULL_BODY_IDX = len(crop_cards)
    all_options = list(range(len(crop_cards) + 1))

    def _crop_label(i):
        if i == FULL_BODY_IDX:
            return f"Full-body crop ({fb_w}×{fb_h})"
        return f"Crop {i + 1} · {crop_cards[i]['label']} (conf={crop_cards[i]['conf']:.2f})"
    ...
    cropped = full_body_crop if selected_idx == FULL_BODY_IDX else crop_cards[selected_idx]["crop"]
...
# ---------- Step 2: BLIP-2 caption for query crop ----------
with st.spinner("Step 2 — Generating caption for query crop with BLIP-2..."):
    t0 = time.time()
    query_caption = generate_caption(cropped, blip2_processor, blip2_model)
    timings["blip2_caption"] = time.time() - t0
st.success(f"Step 2 done — Caption: \"{query_caption}\" ({timings['blip2_caption']:.2f}s)")

# ---------- Step 3: Fused CLIP embedding (image + caption) ----------
with st.spinner("Step 3 — Encoding crop + caption with fine-tuned CLIP, fusing..."):
    t0 = time.time()
    img_emb  = encode_query_image(cropped, clip_model, clip_preprocess)   # tensor
    txt_emb  = encode_caption(query_caption, clip_model)                   # tensor
    query_emb = fuse_embeddings(img_emb, txt_emb, fusion_alpha)            # numpy
    timings["clip_fused"] = time.time() - t0
st.success(f"Step 3 done — Fused embedding α={fusion_alpha} ({timings['clip_fused']:.2f}s)")

# ---------- Step 4: ANN candidate retrieval ----------
rerank_pool = top_k * rerank_factor
with st.spinner(f"Step 4 — HNSW retrieval of top-{rerank_pool} candidates..."):
    t0 = time.time()
...
st.success(f"Step 4 done — Retrieved {len(indices)} candidates ({timings['hnsw']:.2f}s)")

# ---------- Step 5: BLIP-2 ITM re-ranking ----------
st.info(
    f"Step 5 — Re-ranking {len(indices)} candidates with BLIP-2 ITM "
...
st.success(f"Step 5 done — Re-ranked to top-{top_k} ({timings['itm_rerank']:.2f}s)")
...
labels_vals = [
    ("Step 1 — YOLO", t_yolo),
    ("Step 2 — BLIP-2 Caption", timings["blip2_caption"]),
    ("Step 3 — CLIP Fused", timings["clip_fused"]),
    ("Step 4 — HNSW", timings["hnsw"]),
    ("Step 5 — ITM Re-rank", timings["itm_rerank"]),
]
...
st.caption(
    f"Fused query α={fusion_alpha} · Pool: top-{rerank_pool} by cosine sim → re-ranked by BLIP-2 ITM"
)
